...
pub const Panic = struct {
    ...
    pub fn sentinelMismatch(expected: anytype, found: @TypeOf(expected)) noreturn {
        printCurrentTest();
        std.debug.FormattedPanic.sentinelMismatch(expected, found);
    }

    pub fn unwrapError(ert: ?*std.builtin.StackTrace, err: anyerror) noreturn {
        printCurrentTest();
        std.debug.FormattedPanic.unwrapError(ert, err);
    }

    pub fn outOfBounds(index: usize, len: usize) noreturn {
        printCurrentTest();
        std.debug.FormattedPanic.outOfBounds(index, len);
    }

    pub fn startGreaterThanEnd(start: usize, end: usize) noreturn {
        printCurrentTest();
        std.debug.FormattedPanic.startGreaterThanEnd(start, end);
    }

    pub fn inactiveUnionField(active: anytype, accessed: @TypeOf(active)) noreturn {
        printCurrentTest();
        std.debug.FormattedPanic.inactiveUnionField(active, accessed);
    }

    pub const messages = std.debug.FormattedPanic.messages;

    fn printCurrentTest() void {
        if (current_test) |ct| {
